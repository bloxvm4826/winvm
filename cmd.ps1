...
function Respawn {
  Act
  KeyDown 0x1B; Start-Sleep -m 90; KeyUp 0x1B      # Esc opens the Roblox menu
  Start-Sleep 2
  Click 480 681; Start-Sleep 2                     # Respawn
  Click 681 476; Start-Sleep 9                     # confirm
...
function Guard {
  ...
  foreach ($c in @(@(1013, 216), @(617, 275), @(939, 329), @(800, 511), @(894, 524))) {
    Click $c[0] $c[1]
    Start-Sleep -m 700
    if (-not (IsModal)) { Write-Host ("modal closed at {0},{1}" -f $c[0], $c[1]); return $true }
  }
  Write-Host "modal stuck"
  $true
}
...
Respawn
Write-Host ("after drowned=" + (Drowned))
Add-Type -AssemblyName System.Drawing
