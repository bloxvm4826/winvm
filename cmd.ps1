...
function Guard {
  $a = GetPix 800 526          # blue button: Robux "Buy" OR "Join Community"
  if (-not ($a.B -gt 170 -and $a.R -lt 110)) { return $false }
  $d = GetPix 700 205          # inside the Robux dialog panel (dark) / game world for the join popup
  if ($d.R -lt 75 -and $d.G -lt 75 -and $d.B -lt 75) {
    Click 1013 216; Write-Host "closed robux dialog"   # its X
  } else {
    Click 894 524; Write-Host "closed join popup"      # Not Now
  }
  Start-Sleep -m 900
  $a2 = GetPix 800 526
  if ($a2.B -gt 170 -and $a2.R -lt 110) { Click 939 329; Start-Sleep -m 700; Write-Host "fallback close" }
  $true
}
...
(Guard)
Start-Sleep 2
$a = GetPix 800 526; "after: $($a.R),$($a.G),$($a.B)"
Add-Type -AssemblyName System.Drawing
